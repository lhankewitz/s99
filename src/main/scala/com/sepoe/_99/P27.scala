...
object P27 {
  ...
  def main(args: Array[String]): Unit = {  
	  ...
	  funcs2[String].foreach{ f => { 
	 	  val result = f(groupSizes, list)
	 	  println(result.length + " number of combinations.")
	 	  println(result)
	 	  }   	   
	  }	   
  ...
  def funcs[A] = Array[(List[A]) => List[List[List[A]]]](
		  group3_1 _
  		)
  		
  // 	solutions for a)
  def group3_1[A](l:List[A]):List[List[List[A]]] = Nil
  
  

  def funcs2[A] = Array[(List[Int], List[A]) => List[List[List[A]]]](
		  group1 _
  )
  
  // solutions for b)
  def group1[A](spec:List[Int], data:List[A]):List[List[List[A]]] = Nil
}